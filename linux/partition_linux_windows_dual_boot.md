...
La partition 4 est formatée en NTFS et est montée dans le système d'exploitation avec la lettre 'D:' et le label '软件'.
...
- **Taille** : 190 Go.
- **Rôle** : Partition principale où est installé Windows.
...
## 3. Les Volumes
Un volume est une partition formatée et montée, prête à être utilisée par l'utilisateur. Voici les volumes actifs dans ce cas :
...
| Partition    | Volume associé | Utilisation                       |
|--------------|----------------|-----------------------------------|
| Partition 1  | v2 (System)    | Utilisée au démarrage pour initialiser l'OS. |
| Partition 2  | Aucun          | Réservée par le système.          |
...
- **Partition 1 (System)** : Indispensable au démarrage.
- **Partition 2 (Reserved)** : Transparente pour l'utilisateur mais essentielle pour le système.
- **Partitions C: et D:** : Espaces que l'utilisateur utilise directement.
...
- **Raison** :
  - Les disques modernes (SSD ou HDD) utilisent des blocs physiques de 4 Ko.
  - Aligner les partitions sur des multiples de 1 Mo améliore les performances en réduisant les lectures/écritures fragmentées.
...
Les données **GPT** (en-tête et table des partitions) ainsi que la zone d'alignement sont situées avant l'offset de **1024 KB** où commence la partition **P1**.

Si tu veux explorer ou vérifier ces données, tu peux utiliser un outil comme `gdisk` ou `parted` sur Linux, ou `diskpart` sous Windows. Besoin de détails sur ces outils ?
...
1. Réduire la taille de la partition **D:** (partition 4) pour libérer de l’espace non alloué.
2. Utiliser cet espace pour créer les partitions nécessaires à Linux :
...
3. Installer un chargeur d’amorçage comme **GRUB** pour gérer le dual-boot.
...
1. **Ouvrir la Gestion des disques sous Windows** :
   - Appuie sur `Win + R`, tape `diskmgmt.msc` et valide.
2. Clic droit sur la partition **D** (286 Go), puis **Réduire le volume**.
3. Libère un espace suffisant pour Linux :
   - 50-100 Go est un bon choix pour une utilisation standard.
4. Tu obtiens un espace non alloué (non formaté) après **D**.

### B. Installer Linux et créer des partitions :
1. Démarre depuis une clé USB Linux live (ex. **Ubuntu**, **Fedora**).
...
3. Installe **GRUB** sur la partition EFI existante (Partition 1).
...
| **P4**    | NTFS    | 186 GB       | Partition **D:** réduite pour données Windows. |
...
Le BIOS (ou plus précisément l'UEFI dans ton cas, étant donné que tu utilises un disque **GPT**) décide quel chargeur d'amorçage (comme **GRUB** ou le bootloader Windows) doit être lancé en fonction de l'entrée de démarrage UEFI. Voici comment cela fonctionne en détail :

---

## 1. Le rôle de l'UEFI dans le démarrage
L'UEFI remplace l'ancien BIOS traditionnel et fonctionne différemment :
...
- L'UEFI utilise une table appelée **NVRAM (Non-Volatile RAM)** pour enregistrer les entrées de démarrage. Chaque entrée pointe vers un fichier exécutable EFI sur la partition EFI.
...
- Si **Windows Boot Manager** est toujours prioritaire, tu peux changer cela manuellement via l'UEFI ou depuis Linux.
...
1. Démarre avec une clé USB Linux.
2. Monte la partition EFI :
   ```bash
...
1. Ajoute l'entrée de GRUB si elle a disparu :
    ```bash
...
## 1. Où est la NVRAM ?
La **NVRAM (Non-Volatile RAM)** est une mémoire intégrée dans la carte mère, indépendante des disques durs. Elle fait partie du microprogramme UEFI et stocke diverses informations, notamment les entrées de démarrage pour charger les systèmes d'exploitation. 

- La **NVRAM** n'est pas sur le disque, elle est physiquement située dans la puce UEFI de la carte mère.
...
### Sous Windows :
1. Ouvre une invite de commande en mode administrateur.
2. Utilise l'outil intégré BCDEDIT pour voir les entrées UEFI :
...
3. Une fois terminé, démonte la partition :

    sudo umount /mnt

#### Sous Windows :
1. Ouvre l'outil Gestion des disques :
   - Appuie sur Win + R, tape diskmgmt.msc, et valide.
2. Identifie la partition EFI (elle est de petite taille, ex. 100-500 Mo, et formatée en FAT32).
3. Attribue une lettre de lecteur :
   - Clic droit sur la partition EFI > Modifier la lettre de lecteur et les chemins > Ajouter > Choisis une lettre.
4. Une fois la lettre attribuée (ex. Z:), utilise l'Explorateur de fichiers ou l'invite de commande pour accéder à la partition :
